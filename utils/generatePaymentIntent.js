...
export async function generatePaymentIntent(orderId, totalPrice){
    try {
        ...
        await database.query(`
            ...
            VALUES ($1, $2, $3, $4) RETURNING *`, [orderId, "Online", "Pending", paymentIntent.client_secret])

        return {success: true, clientSecret: paymentIntent.client_secret}
    } catch (error) {
        console.error("Error generating payment intent:", error);
        return {success: false, message: error.message || "Failed to generate payment intent"}
    }
